...
- [ ] 6.1 確認頁面載入後能正確從 Google Sheet 抓取資料並渲染所有工具卡片與日誌卡片（需部署 Apps Script 後手動驗證）
- [ ] 6.2 確認 Skill 卡片點擊後能正確開啟對應的 overlay（需部署後手動驗證）
- [ ] 6.3 確認 Web 卡片和日誌卡片點擊後能在新分頁開啟正確連結（需部署後手動驗證）
- [ ] 6.4 確認所有卡片顯示冒險者名稱（需部署後手動驗證）
- [ ] 6.5 確認 API 失敗時顯示友善錯誤訊息（需部署後手動驗證）
