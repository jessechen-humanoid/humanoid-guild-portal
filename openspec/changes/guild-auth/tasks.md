...
- [x] 6.1 在 `script.js` 頂部設定 Google OAuth Client ID 變數與 Apps Script Web App URL 變數（需使用者自行填入）
- [ ] 6.2 驗證完整登入流程：未登入 → 登入畫面 → Google 登入 → 有權限 → 公會頁面
- [ ] 6.3 驗證無權限流程：Google 登入 → email 不在允許名單 → 顯示「你沒有權限登入」
- [ ] 6.4 驗證登出流程：點擊登出 → 清除狀態 → 回到登入畫面
- [ ] 6.5 驗證快取流程：登入後重新整理頁面 → 不用再次登入直接進入公會頁面
